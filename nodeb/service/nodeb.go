...
// mockery --exported --name=NodeAService --case underscore --output ../mocks/nodea
//mockery --exported --name=SqlExecutor --case underscore --output /Users/keshav/go/src/guardianlink/nodea/mocks/db
...
func (nas nodeAServiceImpl) Create(c *gin.Context) (model.Block, error) {
	...
	// Asynchronously notify NodeA and handle failure
	go func(block model.Block) {
		if err := notifyNodeA(block, constant.NodeAURL.ToString()); err != nil {
...
func (nas nodeAServiceImpl) Update(c *gin.Context) (model.Block, error) {
	...
	go func(block model.Block) {
		if err := notifyNodeAUpdate(block, constant.NodeAURL.ToString()); err != nil {
			logger.Errorf("Failed to notify NodeB: %v", err)
			compensateBlockFailure(block, nas)
		} else {
			markBlockAsComplete(block, nas)
...
func notifyNodeA(block model.Block, nodeaURL string) error {
	...
	req, err := http.NewRequest("POST", nodeaURL, bytes.NewBuffer(jsonValue))
	if err != nil {
...
func notifyNodeAUpdate(block model.Block, nodeAURL string) error {
	...
	url := fmt.Sprintf("%s/%v", nodeAURL, block.Height)
